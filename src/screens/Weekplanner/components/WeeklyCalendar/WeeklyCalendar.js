...
const WeeklyCalendar = (props) => {
  ...
  const onDayPress = (weekday) => {
    ...
    if (!hourlyViewActive) {
      generateHoursOfDay(weekday);
    }
    setSelectedDay(weekday);
    setHourlyViewActive(!hourlyViewActive);
  };
  ...
  const renderHourlyView = () => {
    ...
    const hours = [];
    for (let i = 0; i < 24; i++) {
      const time = startOfDay.clone().add(i, "hours");
      hours.push(
        <View key={time.format("HH:mm")} style={styles.hourBlock}>
          <Text style={styles.hourText}>{time.format("HH:mm")}</Text>
          {/* Adicione outros elementos que você deseja mostrar por hora aqui */}
        </View>
      );
    }
    return <>{hours}</>;
